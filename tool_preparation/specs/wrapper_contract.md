...
- Write to files outside tool directory
- Depend on unrelated top-level package extras just to exercise tool-local loading

**Note for Audio Agent Framework**: For simple tools, `model.py` is **recommended but optional**. Many existing tools (e.g., `asr_qwen3`) implement model logic directly in `server.py`. For complex models, the `ModelWrapper` pattern is strongly encouraged.
...
- Call `model.py` methods (or contain direct model logic for simple tools)
- Format responses
...
- Handle business logic (delegate to model.py when present)
...
**Responsibilities**:
- Define public API of tool package
...
**Purpose**: MCP tool registration and runtime configuration

**Status**: Required for Audio Agent Framework integration

**When Required**:
- MCP tool registration via `audio_agent.tools.catalog.loader`
...
- If wrapper smoke is executed, it should verify the tool-local wrapper path only
- Wrapper smoke must not require unrelated top-level `audio_agent` package extras to be installed
...
2. `config.yaml` in tool directory
...
```python
"""
{ToolName} Wrapper for Audio Agent Framework.
